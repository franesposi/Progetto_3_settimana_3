...
console.log('Esercizio 2:', whoIsBigger());
...
*/
function splitMe() {
  const string = "Mi piace la mozzarella"
  return string.split('')
}
...
console.log('Esercizio 4:', deleteOne('Matita', false));
...
*/
const newString = "Ci sono 3 pastelli";
const onlyLetters = function (string) {
  var stringWithoutNumber = "";
  for ( let i = 0; i < string.length; i++ ) {
    const letter = string[i];
    if (isNaN(parseInt(letter))) {
      stringWithoutNumber += letter
    }
  }
  return stringWithoutNumber
}
console.log('Esercizio 5:', onlyLetters(newString));

/* ESERCIZIO 6
  Crea una funzione chiamata "isThisAnEmail" che riceve una stringa come parametro e ritorna true se la stringa è un valido indirizzo email.
*/
function isThisAnEmail() {
  const greatString = 'francesco@esposito.it'
  if (greatString.includes = '@' + '.') {
    return true
  } else {
    return false
  }
...
*/
const numbersOfReps = 3
const numberOfDices = {
  sum,
  values: [dice()]
}
function rollTheDices(numbersOfReps) {
  for (let i = 0; i < numbersOfReps; i++ ) {
    numberOfDices.sum += values;
    numberOfDices.values.push(dice());
  }
...
*/
const myDate = '2009-10-04'
const howManyDays = function(data) {
  today = new Date();
  dates = new Date(data);
  const differenceInMs = today - dates
  const msInDay = (1000 * 60 * 60 * 24)
  const differenceInDays = Math.floor(differenceInMs / msInDay)
  return differenceInDays
}
console.log('Esercizio 9:', howManyDays(myDate));
...
function isTodayMyBirthday() {
  ...
  if (today === myBDAY && month === myBMonth) {
    return true
  } else {
    return false
  }
...
const deleteobj = 'genere'
console.log('Esercizio 11:', deleteProp(gameOfThrones, deleteobj));

/* ESERCIZIO 12
  Scrivi una funzione chiamata "newestMovie" che trova il film più recente nell'array "movies" fornito.
*/
function newestMovie(movies) {
  return movies.reduce(function (film1, film2) {
...
function searchAndDivide(movies, contentString) {
  ...
  const unmacth = movies.filter (movie => !movie.Title.toLowerCase().includes(contentString.toLowerCase()));
  return {match, unmacth}
}
console.log('Esercizio 18:', searchAndDivide(movies, 'lord of'));
/* ESERCIZIO 19
